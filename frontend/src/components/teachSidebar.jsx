...
import { StudentAttendence } from "./StudentAttendance";



const Profile = ({students , classes}) => (
  <div className="p-4 bg-gray-900 text-white rounded-lg h-full w-full">
    ...
    <StudentAttendence students={students} classes={classes} />
...
const Settings = ({classes}) => (
  ...
  {classes.map((class1) => (
       <CometCardDemo key1 ={class1.class_id} id={class1.class_name} name={class1.section} count={class1.total_students} subject = {class1.subject_name} subject_id = {class1.subject_id} flag={true}/>
  ))}
    </div>
  </div>
);
...
export function TeachSidebar() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_URL}/api/teacher/teacher-classes`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ teacher_email: user.email }),
      });
...
}, [selectedStudent]); // only runs when selection changes
  ...
  const links = [
    {
      label: "Students",
      icon: <IconUserBolt />,
      component: <Profile students = {students} classes = {classes}/>,
    },
    {
      label: "Classes",
      icon: <IconSchool />,
      component: <Settings classes = {classes}/>,
    }
  ];
